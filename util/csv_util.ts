...
export type Table<RowKey extends string, SubKey extends string | number> = Record<
  RowKey extends '#' ? number : string,
  Record<RowKey | SubKey, string | undefined>
>;

export type GetTableFunc = {
  <T extends (string | number), K extends string>(
    table: string,
    inputs: [key: K, ...indices: T[]],
  ): Promise<Table<K, T>>;
  <T extends (string | number), K extends string>(
    table: string,
    inputs: (string | number)[],
    outputs?: [key: K, ...indices: T[]],
  ): Promise<Table<K, T>>;
};
...
export const readCsvContent = async (
  content: string | string[],
): Promise<{ keys: string[]; rows: string[][] }> => {
