...
import sys
import shutil
from sty import fg, rs, Style, RgbFg    # used to style colors
...
def build_cacbot():
    ...
    if int(output) > 0:
        try:
            command = [r'& "C:\Program Files\Microsoft Visual Studio\2022\Community\Msbuild\Current\Bin\MSBuild.exe"', "plugin/Cactbot.sln", "/property:Configuration=Release", "/property:Platform=x64"]
            debug_print(" ".join(command))
            output = show_or_hide_terminal(command)
        except Exception:
            pass
