...
Options.Triggers.push({
  ...
  triggers: [
    ...
    },
    {
      ...
      type: 'GainsEffect',
      netRegex: { effectId: '13D6', capture: true },
      delaySeconds: (_data, matches) => parseFloat(matches.duration) - 3.9,
      durationSeconds: 3.9,
      suppressSeconds: 1,
      response: (data, matches, output) => {
        ...
        const players = data.doubleTroubleTrapTargets.map((player) => {
          if (player === data.me)
            return output.you();
    ...
    },
    {
      ...
      netRegex: { effectId: '13D6', capture: true },
      run: (data, matches) => {
        data.doubleTroubleTrapTargets = data.doubleTroubleTrapTargets.filter((target) =>
          ...
          if (data.role === 'tank') {
            ...
            if (config === 'sg3k') {
              if (myElement === 'fire') {
                ...
                const players = data.waterElementPlayers.filter((player) =>
                  data.party.isDPS(player)
                );
